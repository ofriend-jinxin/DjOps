...
from DjOps.tools.Ansible2 import ResultCallback
...
class ANSRunner(object):
    ...
    def __init__(
        ...
        self.inventory = InventoryManager(self.loader, sources=hosts)(hosts)
        self.variable_manager = VariableManager(self.loader, self.inventory)

        self.passwords = passwords or {}

    def run_model(self, host_list, module_name, module_args):
        self.callback = ResultCallback(self.websocket, self.background)
        play_source = dict(
            name="Ansible Ad-hoc",
            hosts=host_list,
...
#     rbt.run_playbook(["192.168.1.34","192.168.1.130"],playbook_path='/etc/ansibles/api/init/system_init.yml')
#     data = rbt.get_model_result()
